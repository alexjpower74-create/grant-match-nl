...
for (const [division, pop] of elsewhere) {
  named.push({ name: `Somewhere else in Division No. ${division}`, type: 'other', csd_code: null, census_division: division, population_2021: pop });
}
...
const summary = `${communities.length} communities (${communities.filter((c) => c.type === 'other').length} "Somewhere else" entries; `
...
if (flags.check) {
  ...
  console.log(`build-reference --check: ${same ? 'communities.json is up to date' : 'communities.json differs from the saved table'}; ${summary}`);
