...
if (robotsRes.status >= 200 && robotsRes.status < 300) {
  ...
  if (!robotsAllows(robotsTxt, USER_AGENT, pathAndQuery))
    fail(`robots.txt at ${target.origin} disallows ${pathAndQuery} for our User-Agent`)
  robots = 'allowed'
...
if (res.url && res.url !== url) {
  ...
  if (!flags['allow-redirect'])
    fail(`${msg}; a page that redirects away is not a source (pass --allow-redirect after checking it, and use the final URL)`)
  const finalPath = `${final.pathname}${final.search}`
  if (final.origin !== target.origin || !robotsAllows(robotsTxt, USER_AGENT, finalPath))
    fail(`${msg}, which robots.txt does not allow or is another host`)
  console.error(`fetch-source: ${msg}`)
}
