...
export function validateProgram(record) {
  ...
  if (sample === true) {
    ...
    if (isStr(record.provider) && !record.provider.startsWith('SAMPLE '))
      bad('provider', 'a SAMPLE program’s provider must start "SAMPLE "')
    ...
    record.sources.forEach((s, i) => {
      {
        ...
        if (!Number.isInteger(s.http_status) || s.http_status < 100 || s.http_status > 599)
          bad(`${p}.http_status`, 'must be an HTTP status code')
        ...
        if (typeof s.text_sha256 !== 'string' || !HEX64_RE.test(s.text_sha256))
          bad(`${p}.text_sha256`, 'must be 64 lowercase hex characters')
  ...
  if (!isObj(intake)) bad('intake', 'must be an object')
    ...
    if (!('deadline' in intake)) bad('intake.deadline', 'required key is missing (null when there is none)')
      ...
      if (intake.status === 'unknown' || intake.status === 'continuous')
        bad('intake.deadline', `a ${intake.status} intake can’t have a deadline`)
    }
  }
  ...
  if (record.cost_share !== null && record.cost_share !== undefined) {
    ...
    if (isObj(c)) {
      if (typeof c.percent !== 'number' || !(c.percent > 0 && c.percent <= 100))
        bad('cost_share.percent', 'must be a number above 0 and at most 100')
  ...
  if (!Array.isArray(record.contacts)) bad('contacts', 'must be an array ([] when no official page gives one)')
    ...
    record.contacts.forEach((c, i) => {
      ...
      if (!(c.email === null || (isStr(c.email) && /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(c.email))))
        bad(`${p}.email`, 'must be an email address or null')
  ...
  function checkRule(path, rule) {
    ...
    const only = (allowed) =>
      keys.forEach((k) => {
        if (!allowed.includes(k)) bad(`${path}.${k}`, `unknown key for a ${kind} rule`)
      })
    const nonEmptyFrom = (k, list) => {
      const v = rest[k]
      if (!Array.isArray(v) || v.length === 0) return bad(`${path}.${k}`, 'must be a non-empty array')
      v.forEach((x, j) => {
        if (!list.includes(x)) bad(`${path}.${k}[${j}]`, `"${x}" is not a known id`)
      })
    ...
    switch (kind) {
      ...
      case 'location': {
        ...
        if (present[0] === 'census_divisions') {
          ...
          if (!Array.isArray(v) || v.length === 0 || !v.every(isDivision))
            bad(`${path}.census_divisions`, 'must be a non-empty list of 1–11')
        }
        if (present[0] === 'communities') {
          const v = rest.communities
          if (!Array.isArray(v) || v.length === 0 || !v.every(isStr))
            bad(`${path}.communities`, 'must be a non-empty list of community ids')
        }
        break
      }
      case 'industry':
