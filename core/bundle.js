...
export function buildBundle({ programs, pageTexts, communities, industries, data_set }) {
  ...
  for (const p of sorted) {
    ...
    if (dataSet === 'real' && p?.sample === true)
      problems.push({ slug, path: 'sample', reason: 'a SAMPLE program can’t go in the real bundle' })
    if (dataSet === 'sample' && p?.sample !== true)
      problems.push({ slug, path: 'sample', reason: 'a real program can’t go in the SAMPLE bundle' })
  }
