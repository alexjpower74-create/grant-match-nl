...
export function quotesOf(record) {
  const out = []
  const add = (path, obj) => {
    if (obj && typeof obj === 'object' && typeof obj.quote === 'string')
      out.push({ path: `${path}.quote`, quote: obj.quote, source: obj.source })
  }
  add('summary', record.summary)
  ;(record.funding_types ?? []).forEach((f, i) => {
    add(`funding_types[${i}]`, f)
  })
...
export function verifyProgram(record, pageTexts = {}) {
  ...
  if (d && typeof d.date === 'string' && typeof d.quote === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(d.date)) {
    if (!d.quote.includes(d.date)) {
      ...
      if (!hasDay || !hasMonth) {
        bad(
          'intake.deadline.date',
          `${d.date} is not written in its quote (needs the day ${dd} and ${month ?? 'the month'}, or the ISO date)`,
        )
      }
    }
  }
