...
function overlaps(a, s) {
  ...
  let lo = a.min
  let loIn = a.min_inclusive
  if (s.min !== null && (s.min > lo || (s.min === lo && !s.min_inclusive))) {
    lo = s.min
    loIn = s.min_inclusive
  }
  // Upper end of the intersection (null = unbounded).
  let hi = a.max
  let hiIn = a.max_inclusive
  if (s.max !== null && (hi === null || s.max < hi || (s.max === hi && !s.max_inclusive))) {
    hi = s.max
    hiIn = s.max_inclusive
  }
...
export function evaluateCriterion(criterion, profile) {
  ...
  switch (rule.kind) {
    ...
    case 'structure': {
      ...
      const accepted = joinWords(
        rule.in.map((id) => labelOf(STRUCTURES, id)),
        'or',
      )
    ...
    case 'ownership': {
      ...
      const wanted = joinWords(
        rule.any.map((id) => labelOf(OWNERS, id)),
        'or',
      )
    ...
    case 'purpose': {
      const wanted = joinWords(
        rule.any.map((id) => labelOf(PURPOSES, id)),
        'or',
      )
    ...
    case 'years_operating': {
      ...
      return bandResult(
        ...
        rule,
        scale,
        `Your time in business (${band.label.toLowerCase()})`,
    ...
    case 'project_cost': {
      if (!profile.cost || profile.cost === 'unsure') {
        return result(
          'unknown',
          profile.cost === 'unsure' ? "You weren't sure of the project cost." : "You didn't give a project cost.",
          'not_answered',
        )
      }
...
function bandResult(interval, rule, scale, subject, fmt) {
  ...
  if (cmp === 'missed')
    return rule.normally ? normallyResult() : result('missed', `${subject} is outside the page's limit. The page says ${limit}.`)
...
export function localDate(instant) {
  const parts = new Intl.DateTimeFormat('en-CA', { timeZone: TIME_ZONE, year: 'numeric', month: '2-digit', day: '2-digit' }).formatToParts(
    toDate(instant),
  )
...
export function evaluateProgram(program, profile, { now, sourceStatus = {} } = {}) {
  ...
  const fundingTypes = program.funding_types.map((f) => ({
    type: f.type,
    label: labelOf(FUNDING_TYPES, f.type),
  ...
  return {
    ...
    cost_share: program.cost_share
      ? { percent: program.cost_share.percent, text: program.cost_share.text, ...q(program.cost_share) }
      : null,
    contacts: program.contacts.map((c) => ({
      label: c.label,
      phone: c.phone ?? null,
      email: c.email ?? null,
      census_divisions: c.census_divisions ?? null,
      ...q(c),
    })),
    criteria,
    counts,
...
function fitFor({ criteria, intake, verification }) {
  ...
  if (verification.stale)
    why.push(`Last verified ${verification.age_days} days ago, more than ${STALE_DAYS} days. Check the official page.`)
...
export function topMatchOf(criteria) {
  ...
  const best = [...met].sort(
    (a, b) =>
      (a.quote.length > SHORT_QUOTE) - (b.quote.length > SHORT_QUOTE) ||
