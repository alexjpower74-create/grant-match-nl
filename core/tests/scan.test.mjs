  ...
  server = http.createServer((req, res) => {
    if (req.method === 'POST' && req.url === '/api/admin/checks') {
      ...
      req.on('end', () => {
        ...
        if (req.headers.authorization !== 'Bearer test-admin-token') { res.writeHead(401); res.end('{"error":"no"}'); return; }
...
test('--dry: summary names the missing quote, latest.json is written, raw pages are cached, nothing is POSTed', async () => {
  ...
  assert.deepEqual(latest.sources.map((s) => s.source_id), ['nl-sample-growth-grant--main', 'nl-sample-growth-grant--contact']);
...
test('without --dry the result is POSTed with the Bearer token and the stored id is reported', async () => {
  ...
  assert.deepEqual(body.sources.map((s) => s.source_id), ['nl-sample-community-fund--main']);
...
test('the token can come from a .dev.vars file; a missing token stops before any fetch; a 401 exits 1', async () => {
  ...
  posts.length = 0;
  const fromFile = await scan(['--sample', '--only', 'nl-sample-community-fund', '--scans', path.join(TMP, 'file'), '--worker', origin, '--dev-vars', devVars]);
  ...
  const wrong = await scan(['--sample', '--only', 'nl-sample-community-fund', '--scans', path.join(TMP, 'wrong'), '--worker', origin], { GM_ADMIN_TOKEN: 'wrong' });
