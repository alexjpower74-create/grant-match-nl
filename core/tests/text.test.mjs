...
test('pageText: a tag ends at the first > outside a quoted attribute value', () => {
  ...
  const escaped =
    '<div class="cmp" data-cmp-data-layer="{&quot;h&quot;:&quot;&lt;p&gt;Duplicate sentence.&lt;/p&gt;&quot;, &quot;x&quot;: &quot;&gt;&quot;}">Real sentence.</div>'
...
test('blockText: block edges become one \\n, everything else as pageText', () => {
  ...
  assert.equal(
    blockText('<td>a</td><td>b</td><x-card>c</x-card>'),
    'a\nb\nc',
    'a run touching an edge is one \\n even with other tags in it',
  )
  ...
  for (const html of [
    '<p>One</p><p>Two</p>',
    '<p>a&nbsp;  b</p><div>​c﻿</div>',
    '<nav><a>Home</a> &gt; <a>Funding</a>:</nav><p>Closed.</p>',
...
test('pageText: head, script, style, noscript, template, svg and comments are gone', () => {
  const html =
    '<html><head><title>Title</title></head><body><script>var a = "<p>no</p>";</script><style>p{}</style>' +
...
test('contextFor gives the rest of the sentence, never menu text', () => {
  const text =
    'Home Funding Contact us Programs Who can apply. Owners must live here and the business must operate in NL; it can be seasonal. Next sentence here.'
...
test('contextFor on blockText stops at block edges too; \\n never in the output', () => {
  ...
  const crumb =
    '<nav><a href="/">Home</a> &gt; <a href="/funding/">Funding: SAMPLE grants</a></nav><div class="alert"><p>The SAMPLE Grant is not accepting applications.</p></div>'
  assert.deepEqual(
    contextFor(pageText(crumb), 'The SAMPLE Grant is not accepting applications.'),
    { before: 'SAMPLE grants ', after: '' },
    'the old leak',
  )
...
test("real pages: CanExport's closed intake has no breadcrumb before it; no context of any real quote holds \\n", () => {
  ...
  assert.ok(
    !/Trade Commissioner Service|Our solutions|CanExport SMEs $/.test(intake.before),
    `breadcrumb in before: ${JSON.stringify(intake.before)}`,
  )
