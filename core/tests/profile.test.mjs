...
test('missing or unknown answers are plain-English errors per field', () => {
  ...
  const bad = parseProfile({ community: 'atlantis', industry: '99', structure: 'llc', employees: '-3', years: 'ages', revenue: 'lots', purposes: 'fun', cost: 'cheap', owners: 'aliens' }, reference());
  assert.deepEqual(bad.errors.map((e) => e.field), ['community', 'industry', 'structure', 'employees', 'years', 'revenue', 'owners', 'purposes', 'cost']);
  for (const e of bad.errors) assert.match(e.message, /^[A-Z].*\.$/);

  const empty = parseProfile({}, reference());
  assert.deepEqual(empty.errors.map((e) => e.field), ['community', 'industry', 'structure', 'employees', 'years', 'revenue', 'purposes']);
  assert.ok(parseProfile({ ...Object.fromEntries(new URLSearchParams(AUTO_QUERY)), name: 'x'.repeat(81) }, reference()).errors.some((e) => e.field === 'name'));
});
