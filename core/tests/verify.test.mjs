...
test('schema: SAMPLE naming, enums, unique criterion ids, rule shapes', () => {
  ...
  const cases = [
    ...
    [(r) => { r.sources[0].url = 'https://www.gov.nl.ca/x'; }, 'sources[0].url:'],
    [(r) => { r.sample = false; }, 'url:'],
    [(r) => { r.level = 'city'; }, 'level:'],
    [(r) => { r.intake.status = 'maybe'; }, 'intake.status:'],
    ...
    [(r) => { r.extra = 1; }, 'extra:'],
    [(r) => { r.sources[0].fetched_at = '2026-09-01 12:00'; }, 'sources[0].fetched_at:'],
    [(r) => { r.sources[0].id = 'other--main'; }, 'sources[0].id:'],
  ];
...
test('quotesOf lists every quote with its path', () => {
  ...
  assert.deepEqual(paths, ['summary.quote', 'funding_types[0].quote', 'intake.quote', 'intake.deadline.quote', 'max_amount.quote',
    'cost_share.quote', 'criteria[0].quote', 'criteria[1].quote', 'criteria[2].quote']);
});
