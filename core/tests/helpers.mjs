...
export const AUTO_QUERY = 'name=SAMPLE Auto Service&community=grand-falls-windsor&industry=81&structure=corporation&employees=6&years=10plus&revenue=500k_1m&owners=none&purposes=equipment,digital&cost=25k_50k';
export const DAYCARE_QUERY = 'name=SAMPLE Daycare&community=gander&industry=62&structure=sole_proprietor&employees=4&years=1to2&revenue=100k_300k&owners=women&purposes=hire,training&cost=10k_25k';
...
export async function sampleBundlePrograms() {
  ...
  const { bundle, problems } = buildBundle({ programs: samplePrograms(), pageTexts: await samplePageTexts(), communities, industries, data_set: 'sample' });
