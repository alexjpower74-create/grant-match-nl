...
test('a URL shared by several sources is fetched once per run, and every source gets the result', async () => {
  ...
  const result = await runChecks(
    options({
      programs: [growth, copy],
      onRaw: async (r) => {
        raws.push(r.source_id)
      },
    }),
  )
  ...
  assert.deepEqual(raws, [
    ...
    'nl-sample-growth-copy--main',
    'nl-sample-growth-copy--contact',
  ])
})
...
test('sourceStatusFrom folds runs oldest first; a failed check keeps the missing count and page_gone', () => {
  ...
  assert.deepEqual(afterTimeout['a--main'], {
    last_checked_at: '2026-09-21T10:15:00.000Z',
    last_ok: false,
    last_verified_at: '2026-09-07T10:15:00.000Z',
    missing_quotes: 1,
    page_gone: false,
  })
  ...
  assert.deepEqual(backAgain['a--main'], {
    last_checked_at: '2026-09-21T10:15:00.000Z',
    last_ok: true,
    last_verified_at: '2026-09-21T10:15:00.000Z',
    missing_quotes: 0,
    page_gone: false,
  })
})
